/**
 * Premier League 2026/27 Predictor — Google Sheet backend (with PINs)
 * ------------------------------------------------------------------
 * Stores predictions and results, and serves them back for the leaderboard.
 * This is a SEPARATE sheet from the World Cup game. Do not point the PL app at
 * the World Cup sheet, or the two leaderboards will read each other's rows.
 *
 * The stage tag is the gameweek: "GW1", "GW2", ... "GW38". Each player has one
 * prediction row per gameweek; results live under the reserved name
 * "__RESULTS__" (type=result), one row per gameweek. The page always reads the
 * latest row per stage|name|type, so re-submitting simply overwrites.
 *
 * PINs (self-service): the first time a name submits, the PIN it sends is saved
 * as that name's PIN. After that, any write under that name must send the
 * matching PIN. The organiser name "__RESULTS__" sets the organiser PIN the
 * first time results are saved; that PIN also works as an admin override (used
 * when the organiser imports someone's backup code).
 *
 * PINs live in a separate "pins" tab and are NEVER served back to the page.
 *
 * You do NOT need to understand this. After pasting it in, deploy:
 *   Deploy > New deployment > Web app > Execute as: Me, Access: Anyone > Deploy
 * To update later: Deploy > Manage deployments > edit (pencil) > New version > Deploy
 */
